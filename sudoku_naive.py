...
def main():
    ...
    board = setup()
    title = (sys.argv[3])[:-8] + "solved"
    count = 0
    empties = findEmpties(board)
    solve (board, empties, count)
    print(title)
    outputFile.write(title + "/n")
    for i in range(9):
        output = str(board[i : i + 9])[1 : -1]
        print (output)
...
def setup():
    unformattedLines = open(sys.argv[1], 'r').read().split('\n')
    # print (unformattedLines)
    boardIndex = unformattedLines.index(sys.argv[3])
    unformattedBoard = unformattedLines[boardIndex]
    board = []
    boardArray = unformattedLines[boardIndex + 1 : boardIndex + 10]
...
def solve (board_arr, empties, count):
    count = count + 1
    pos = 0
    if len(empties) == 0:
        print(count)
    ...
    for guess in range(1,10):
        if isValid(board_arr, guess, pos):
            board_arr[pos] = guess
            if solve(board_arr, empties[1:], count):
                return True
            board_arr[pos] = "_"
    return False

def isValid(board_arr, guess, pos):
    for i in neighbors[pos]:
        if board_arr[i] == guess and i != pos:
            return False
    return True
