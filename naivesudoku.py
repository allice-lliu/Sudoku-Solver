...
def validBoard(board, index, num):
    neighbors = {
...
def formatBoard(boardName, board):
    final = boardName.replace('unsolved', 'solved') + '\n'
    n = 0
    while (n < 9):
        for m in range(0, 9):
            final += str(board[n*9 + m]) + ', '
        final += '\n'
        n += 1
    return final

def solveBoard(board, blanks, backtracks):
    index = 0
    if len(blanks) == 0:
        print(backtracks)
        return True
    ...
    for num in range(1,10):
        if validBoard(board, index, num):
            board[index] = num
            if solveBoard(board, blanks[1:], backtracks + 1):
                return True
            board[index] = -1
    return False
...
def main():
    ...
    solveBoard(board, findBlanks(board), backtracks)

    final = formatBoard(boardName, board)
    fout.write(final)
    fout.close()
